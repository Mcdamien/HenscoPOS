...
export async function GET() {
  try {
    ...
    console.log('Stats calculation starting, allowed shops:', ALLOWED_SHOPS)
    ...
    try {
      ...
      totalProfit = transactionItems
        .filter(item => ALLOWED_SHOPS.includes(item.transaction.store.name as any))
        .reduce((sum, item) => sum + ((item.itemPrice - item.itemCost) * (item.qty || 0)), 0)
    } catch (profitError) {
