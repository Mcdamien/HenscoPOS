...
export function useKeyboardNavigation({
  ...
  const registerField = useCallback((index: number) => {
    return (element: HTMLElement | null) => {
      ...
      // ONLY if nothing else is currently focused (like on initial load)
      // or if the element itself is already focused (to maintain it)
      if (element && index === focusedIndex) {
        const activeEl = document.activeElement
        const isNeutralFocus = !activeEl || activeEl === document.body
        
        if (isNeutralFocus) {
          setTimeout(() => {
            if (fieldRefs.current[index] === element) {
              element.focus()
              if (element instanceof HTMLInputElement) {
                element.select()
              }
